...
/**
 * Hero2Section — mobile optimised, full-screen on desktop,
 * progressive load on mobile, includes accent "&"
 */
export function Hero2Section({ section, image, serviceTitle }: ImageSectionProps) {
  ...
  // --- Image setup ---
  const folder = image?.folder ?? serviceTitle?.toLowerCase().replace(/\s+/g, "-") ?? "home";
  ...
  return (
    <section
      className="relative w-full h-[70vh] sm:h-[90vh] md:h-[100vh] min-h-[500px] flex items-center justify-center text-white overflow-hidden"
      style={{ contain: "paint" }}
    >
