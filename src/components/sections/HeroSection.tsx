...
export function HeroSection({ section, image, serviceTitle }: ImageSectionProps) {
  ...
  // --- Folder + Files ---
  const folderName = image?.folder ?? "home";
  const desktopLowResFile = image?.desktopLowRes ?? "home-hero-desktop-lowres.webp";
  const desktopFallbackFile = image?.desktopFallback ?? "home-hero-desktop-fallback.webp";
  const videoFile = image?.videoFile ?? "acoustic-design-places-we-work-learn-live2.mp4";
  const mobileLowResFile = image?.mobileLowRes ?? "home-hero-mobile-lowres.webp";
  const mobileFile = image?.mobileFile ?? "home-hero-mobile.webp";
  ...
  const imgMobile = `/images/${folderName}/${mobileFile}`;
